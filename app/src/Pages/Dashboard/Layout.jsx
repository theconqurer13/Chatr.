    ...
    return (
        ...
        <div className="flex h-screen bg-gray-100 font-sans">
            ...
            <div className="flex flex-col bg-gray-900 text-white shadow-lg transition-all duration-300 w-20 md:w-64">
                ...
                {/* Navigation */}
                <nav className="flex-1 p-1 sm:p-2 md:p-4">
                    <ul className="flex flex-col gap-1 sm:gap-2 list-none">
                        {menuItems.map((item) => {
                            ...
                            return (
                                <li key={item.id}>
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-2 px-2 sm:px-4 py-2 sm:py-3 rounded-lg transition-all duration-200 text-left justify-center md:justify-start ${
                                            isActive
                                    ...
                                    >
                                        ...
                                        <span className="font-medium text-sm sm:text-base hidden md:inline">{item.label}</span>
                                    </button>
                                </li>
                            );
                        })}
                ...
                {/* Logout */}
                <div className="p-1 sm:p-2 md:p-4 border-t border-gray-700">
                    <button
                        onClick={logout}
                        className="w-full flex items-center gap-2 px-2 sm:px-4 py-2 sm:py-3 text-gray-300 hover:bg-gray-700 hover:text-white rounded-lg transition-colors duration-200 justify-center md:justify-start"
                        title="Logout"
                    >
                        <LogOut size={18} className="flex-shrink-0" />
                        <span className="font-medium text-sm sm:text-base hidden md:inline">Logout</span>
                    </button>
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              ...
              {/* --- Content Area --- */}
              <main className="flex-1 overflow-y-auto p-1 md:p-8 bg-gradient-to-b from-gray-900 to-gray-800 webkit-scrollbar-hidden">
                <Outlet />
              </main>
            </div>
        </div>
