        ...
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-violet-500 focus:border-transparent text-lg"
            />
          </div>
          ...
          {/* Filters - Mobile */}
          {showFilters && (
            <div className="md:hidden bg-gray-50 rounded-xl p-4 space-y-2 animate-fadeIn">
              {filters.map(filter => (
                <button
                  key={filter}
                  onClick={() => handleFilterToggle(filter)}
          ...
          <div key={user.id}
           ...
           className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
             <div className="flex items-start gap-4 mb-4">
              <img
                src={`https://placehold.co/50x50/violet/white?text=${user.avatar}`}
                alt={user.name}
                className="w-12 h-12 rounded-full flex-shrink-0"
              />
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-gray-900 text-lg">{user.name}</h3>
                <p className="text-violet-600 font-medium">{user.role}</p>
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2 pt-2">
                <button className="flex-1 bg-violet-600 text-white py-2 px-4 rounded-lg hover:bg-violet-700 transition-colors text-sm font-medium flex items-center justify-center gap-2">
                  <MessageCircle size={14} />
                  Message
                </button>
                <button className="flex-1 border border-gray-300 text-gray-700 py-2 px-4 rounded-lg hover:bg-gray-50 transition-colors text-sm font-medium flex items-center justify-center gap-2">
                  <Plus size={14} />
                  Connect
                </button>
              </div>
